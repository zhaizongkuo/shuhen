...
//
// 2026-09-06 起 src/ 全覆盖，豁免表为空。
// 再加新目录时记住：一次加一个，别等「全部修好再一起开」——那天不会到来。
const GUARDED = [
  'src/library',
  'src/core',
  'src/content',
...
const EXEMPT = new Map([
  // 空的。加任何一条都必须写明「为什么还没修」和「怎么修」，
  // 否则豁免表会变成永久的 —— 那时守卫还在跑，但守的东西越来越少。
]);
...
function stripComments(text) {
  return text
    ...
    .map((line) => {
      ...
      // 开发者日志不算界面。console 里的中文是给自己看的，
      // 翻译它只会让日志更难读，而用户根本看不到 ——
      // 除非他打开 DevTools，那时中文比没有信息强。
      // 代价：同一行里既有 console 又有界面文案时会被放过，
      // 但那种写法本身就该在 review 时挡掉。
